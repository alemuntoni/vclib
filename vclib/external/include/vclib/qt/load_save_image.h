...
#ifndef VCL_QT_LOAD_SAVE_IMAGE_H
#define VCL_QT_LOAD_SAVE_IMAGE_H

#include <QImage> // must be included before any other file here
    ...
    int&               h)
{
    ...
    if (!img.isNull()) {
        ...
        std::copy(
            img.bits(),
            img.bits() + img.width() * img.height() * 4,
            ptr.get());
...
#endif // VCL_QT_LOAD_SAVE_IMAGE_H
