...
class OffLoadMeshAction : public LoadMeshAction
{
public:
    using LoadMeshAction::load;

    std::string name() const override { return "Load Off Mesh"; }

    std::shared_ptr<Action> clone() const override
    {
        return std::make_shared<OffLoadMeshAction>(*this);
    }
...
#endif // VCL_PROCESSING_ACTIONS_LOAD_MESH_OFF_LOAD_MESH_ACTION_H
